...
@section('page-header')
  ...
  <div class="breadcrumb-header justify-content-between">
    <div class="left-content">
      <div>
        <h2 class="main-content-title tx-24 mg-b-1 mg-b-lg-1">@yield('header')</h2>
      </div>
    </div>
  </div>
  <!-- /breadcrumb -->
          ...
          <div class="table-responsive">
            <table id="yajra" class="table key-buttons text-md-nowrap hover order-column cell-border	 stripe	">
              ...
              <tr>
                ...
                <th class="border-bottom-0">تاريخ الإنتهاء</th>
                <th class="border-bottom-0">حالة المراجعة</th>
...
@section('ajax')
  ...
  <script type="text/javascript">
    ...
    {{--    DATATABLE   --}}
    {{-- GETTING DATA OF A SPECIFIC AD WHEN EDIT BUTTON IS CLICKED--}}
    ...
    {{--   Editing sub category option according to selected parent   --}}
    ...
    {{--   Editing city option according to selected parent   --}}
    ...
    $(document).on('click', '.approveRecord', function () {
      let id = $(this).data('id');
      let approveConfirm = confirm("هل انت متأكد من الموافقة على الإعلان ؟");
      if (approveConfirm === true) {
        $.ajax({
          url: '{{route('advertisements.approve')}}',
          type: 'post',
          data: {_token: CSRF_TOKEN, id:id},
          beforeSend: function () {
          ...
          success: function () {
            table.ajax.reload();
            $('#successDiv').show().text('تم الموافقة على الإعلان بنجاح')
          },
          error: function () {
